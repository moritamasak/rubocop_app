class AssignsController < ApplicationController
  ...
  def assign_destroy(assign, assigned_user)
    if  assigned_user == assign.team.owner  # 本人とオーナーが同じ
      I18n.t('views.messages.cannot_delete_the_leader') # リーダーは削除できません
    elsif Assign.where(user_id: assigned_user.id).count == 1 # アサインユーザーの数が一つ
      I18n.t('views.messages.cannot_delete_only_a_member') # このチームにしか所属していないので削除できません
    elsif current_user.id == assign.team.owner.id || current_user.id == assigned_user.id # チームオーナー
      assign.destroy
      set_next_team(assign, assigned_user)
      I18n.t('views.messages.delete_member') # メンバーを削除しました
    elsif current_user.id != assigned_user.id
      I18n.t('views.messages.only_leader_can_delete_a_member') # リーダーだけが削除できます。
    else
